# OpenTelemetry Instrumentation in Clarisse-Agent-Framework

This document provides an overview of the OpenTelemetry instrumentation setup in the Clarisse-Agent-Framework.
...
Use the environment variable `Clarisse_FRAMEWORK_INSTRUMENTATION_ENABLED` to enable or disable instrumentation.

```bash
# Enable instrumentation
export Clarisse_FRAMEWORK_INSTRUMENTATION_ENABLED=true
# Ignore sensitive keys from collected events data
export INSTRUMENTATION_IGNORED_KEYS="apiToken,accessToken"
```

If `Clarisse_FRAMEWORK_INSTRUMENTATION_ENABLED` is false or unset, the framework will run without instrumentation.
...
const tracer = trace.getTracer("Clarisse-agent-framework");
...
Running the Instrumented Application (`examples/agents/Clarisse_instrumentation.js`) file.

```bash
## the telemetry example is run on built js files
yarn start:telemetry ./examples/agents/Clarisse_instrumentation.ts
```
