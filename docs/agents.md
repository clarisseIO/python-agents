...
## Implementation in Clarisse Agent Framework
...
Our Clarisse Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new ClarisseAgent({
  llm: new OllamaChatLLM(), // for more explore 'Clarisse-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'Clarisse-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'Clarisse-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/Clarisse_advanced.ts) to see how to modify them.
