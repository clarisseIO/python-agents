...
> [!TIP]
>
> Location within the framework `Clarisse-agent-framework/logger`.
...
In the Clarisse Agent Framework, the [Logger](/src/logger/logger.ts) class is an abstraction built on top of the popular [pino](https://github.com/pinojs/pino) logger, offering additional flexibility and integration.
...
```ts
import { Logger, LoggerLevel } from "Clarisse-agent-framework/logger/logger";

// Configure logger defaults
Logger.defaults.pretty = true; // Pretty-print logs (default: false, can also be set via ENV: Clarisse_FRAMEWORK_LOG_PRETTY=true)
Logger.defaults.level = LoggerLevel.TRACE; // Set log level to trace (default: TRACE, can also be set via ENV: Clarisse_FRAMEWORK_LOG_LEVEL=trace)
...
import { Logger } from "Clarisse-agent-framework/logger/logger";
import { Emitter } from "Clarisse-agent-framework/emitter/emitter";
...
If you need to integrate your own `pino` instance with the Clarisse Agent Framework Logger, you can do so easily. Below is an example that demonstrates how to create a pino logger and use it with the framework’s [Logger](/src/logger/logger.ts).
